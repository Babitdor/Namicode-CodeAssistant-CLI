...
def _is_safe_path(path: Path, base_dir: Path) -> bool:
    ...
    Example:
        >>> base = Path("/home/user/.nami/skills")
        >>> safe = Path("/home/user/.nami/skills/web-research/SKILL.md")
        >>> unsafe = Path("/home/user/.nami/skills/../../.ssh/id_rsa")
...
def list_skills(
    *,
    ...
    user skills will override them. Supports multiple project skill directories
    (e.g., .claude/skills and .nami/skills).

    Args:
        user_skills_dir: Path to the user-level skills directory.
        project_skills_dir: Path to a single project-level skills directory (deprecated).
        project_skills_dirs: List of project-level skills directories (preferred).
    ...
    if project_skills_dirs:
        project_dirs.extend(project_skills_dirs)
    if project_skills_dir:
        project_dirs.append(project_skills_dir)

    # Load project skills second (override/augment)
    # Later directories in the list take precedence
    for proj_dir in project_dirs:
        project_skills = _list_skills(proj_dir, source="project")
