...
def _get_backend(
    backend: BACKEND_TYPES, runtime: ToolRuntime[None, Any]
) -> BackendProtocol:
...
def _get_virtual_mode(backend: BackendProtocol) -> bool:
    """Get the virtual_mode setting from a backend.

    Checks the backend's virtual_mode attribute. For CompositeBackend,
    checks the default backend's virtual_mode.

    Args:
        backend: The backend to check.

    Returns:
        True if in virtual mode (reject Windows paths), False otherwise.
        Defaults to True if virtual_mode is not set.
    """
    virtual_mode = getattr(backend, "virtual_mode", None)
    if virtual_mode is None and hasattr(backend, "default"):
        virtual_mode = getattr(backend.default, "virtual_mode", True)  # type: ignore
    return virtual_mode if virtual_mode is not None else True
...
def _ls_tool_generator(
    ...
    def sync_ls(runtime: ToolRuntime[None, FilesystemState], path: str) -> str:
        ...
        validated_path = _validate_path(
            path, virtual_mode=_get_virtual_mode(resolved_backend)
        )
...
def _grep_tool_generator(
    ...
    def sync_grep(
        ...
        # truncate_if_too_long preserves input type - since formatted is str, result is str
...
def _execute_tool_generator(
    ...
    def sync_execute(
        ...
        # Cast to SandboxBackendProtocol since we've verified it supports execution
        sandbox_backend = cast("SandboxBackendProtocol", resolved_backend)
...
class FilesystemMiddleware(AgentMiddleware):
    ...
    Example:
        ...
        from nami_deepagents.backends import StateBackend, StoreBackend, CompositeBackend
    ...
    def _get_backend(self, runtime: ToolRuntime[None, Any]) -> BackendProtocol:
        """Get the resolved backend instance from backend or factory.
    ...
    def wrap_model_call(
        ...
        has_execute_tool = any((tool.name if hasattr(tool, "name") else tool.get("name")) == "execute" for tool in request.tools)  # type: ignore

        backend_supports_execution = False
        if has_execute_tool:
            # Resolve backend to check execution support
            backend = self._get_backend(request.runtime)  # type: ignore[arg-type]
            backend_supports_execution = _supports_execution(backend)

            # If execute tool exists but backend doesn't support it, filter it out
            if not backend_supports_execution:
                filtered_tools = [tool for tool in request.tools if (tool.name if hasattr(tool, "name") else tool.get("name")) != "execute"]  # type: ignore
        ...
        if system_prompt:
            # Build new system message by combining with existing if present
            ...
            request = request.override(system_message=SystemMessage(content=new_text))
    ...
    def _process_large_message(
        ...
        # This method should only be called when tool_token_limit_before_evict is not None
        assert (
            self.tool_token_limit_before_evict is not None
        ), "tool_token_limit_before_evict must not be None"
